...
class Trashlogger(Node):  # inheritance from node, making it a ros 2 node
    ...
    def __init__(self):
        ...
        self.conn = sqlite3.connect(
            "trash_log.db", check_same_thread=False
    ...
    def check_trash_cluster(
        self, treshhold=0.1
    ):  # for checking if more robots are needed (more that 5 trash in one cluster)
        ...
        for i in range(
            len(data)
        ):  # checks the location of all the trash in the db and sees whether more than 5 have the same coordinates (within the range of the treshold (can be adjuested if needed))
            count = 1
            x1, y1 = data[i]
    ...
    def odom_callback(self, msg):  # runs everytime a new odom message come in
        if (
            self.latest_trash_type is None
        ):  # if the trash type is not received, then the database do not log
            return
        ...
        self.get_logger().info(
            f"Logged trash: {self.latest_trash_type} at ({x:.2f}, {y:.2f}, {z:.2f})"
        )
        ...
        for x, y, trash in data:  # go through all of the data
            if trash not in grouped:  # see if it is a new type of trash
                grouped[trash] = {
                    "x": [],
                    "y": [],
                }  # if yes create an empty list for the coordinates
            grouped[trash]["x"].append(
                x * 100
            )  # add x coordinates *100 cus it's inniatially in meters
            grouped[trash]["y"].append(
                y * 100
            )  # add y coordinates *100 cus it's inniatially in meters

        ax.clear()
        for (
            trash,
            coordinates,
        ) in grouped.items():  # go throught the group of trash type and its coordinates
            ax.scatter(
                coordinates["x"], coordinates["y"], color=colors.get(trash, "black"), label=trash
            )

        ax.set_xlim(0, 180)  # set x limit
        ax.set_ylim(0, 180)  # set y limit
...
def main(args=None):
    ...
    plot_thread = threading.Thread(
        target=live_plot_loop, args=("trash_log.db", node.lock), daemon=True
    )
    plot_thread.start()
